...
void QLearning::train(std::mt19937_64 & generator, const std::vector<double> & s, const int & a, double & r, const std::vector<double> & sPrime, const bool & sPrimeTerminal) {
	...
	if (!sPrimeTerminal) {
		...
		target = r + gamma * maxQ(phiPrime);
	}
	else{
		target = r;
	}
	double TD_error = target - dot(w[a], phi);
	std::vector<double> update(phi);
	for (auto& u_s : update)
		u_s *= (alpha * TD_error);
	std::transform(w[a].begin(), w[a].end(), update.begin(), w[a].begin(),
		[](double w, double u_s)->double{return w + u_s;});
...
int QLearning::getAction(const std::vector<double> & s, std::mt19937_64 & generator) {
	...
	return bestActions[(uniform_int_distribution<int>(0, (int)bestActions.size() - 1))(generator)];	// There are many best actions. Select one uniformly randomly from bestActions.
}
